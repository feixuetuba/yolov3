...
with open(opt.ignore, "r") as fd:
    prune_cfg = yaml.load(fd, yaml.FullLoader)
    ignore = prune_cfg["ignore"]
    triggers = prune_cfg["trigger"]
...
CBL_idx, Conv_idx, prune_idx= parse_module_defs(model.module_defs, ignore)
...
pruned_model = prune_model_keep_size(model, prune_idx, CBL_idx, CBLidx2mask, triggers)
